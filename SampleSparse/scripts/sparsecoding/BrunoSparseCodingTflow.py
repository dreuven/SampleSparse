...
class TensorSparse:
    ...
    def energy_function_func(self):
        ...
        return to_ret
    def energy_value_for_each_sample(self):
        reconstruction = tf.matmul(self.phis, self.a_matr)
        squared_error = (self.data - reconstruction) ** 2.0
        norm_array = tf.abs(self.a_matr)
        ...
        to_ret = left_side + a_value_summed
        return self.sess.run(to_ret, feed_dict = {self.phis:self.phis_so_far, self.data:self.loaded_data})
    ...
    def update_phis(self, ii):
        ...
        if ii % 100 == 0:
            ...
            print("Done pickling")
            output.close()
            self.reconstruction = self.sess.run(tf.matmul(self.phis, self.a_matr), feed_dict = {self.phis : self.phis_so_far})
